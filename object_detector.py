...
def get_detections(model_result):
    ...
    for i in range(count):
        detections.append(
            Detection(title=get_class_name(model_result, i),
                      confidence=get_confidence(model_result, i),
                      location=get_bounding_box(model_result, i)))
    return detections
...
if __name__ == "__main__":
    ...
    image_path = '/home/stefan/Documents/ObjectDetection/Images/generated_sets/generated_480x640/test/1277108163Image000075.jpg'
    img = load_image_into_numpy_array(image_path)
    img = np.expand_dims(img, 0)
    print(run_detection(detector_model, img))
